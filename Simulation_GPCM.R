# ==============================================================================
# GPCM Category Collapse Simulation Study
# ==============================================================================
...
#          sparse response categories in polytomous items under the Generalized
#          Partial Credit Model (GPCM). Examines parameter recovery, bias, and model
...
set.seed(13) # Keep item parameters are the same across replications

# a parameter

a = rlnorm(L,.4,.5) #scaling parameter is needed here for mirt
...
for (N in n){
  ...
  for (r in 1:R){
    ...
    resp_full = simdata(matrix(a),cbind(0,d),itemtype = "gpcm",Theta=matrix(theta))
    
    ## Estimate the Baseline model ----
    md = mirt(resp_full,1,itemtype = "gpcm", SE = TRUE, verbose = F,
    ...
    md2 = mirt(resp_collapsed_UP,1,itemtype = "gpcm", SE = TRUE,  verbose = F,
               technical=list(NCYCLES=2000, message = F))
    ...
    md3 = mirt(resp_collapsed_DOWN,1,itemtype = "gpcm", SE = TRUE,  verbose = F,
               technical=list(NCYCLES=2000, message = F))
...
true_item_param_5 = as.data.frame(gpcm_ip)
...
## 5% Collapse Item Parameter Recovery ----
...
L = 12 #test length
m = 5 # number of categories
n = c(150, 200, 250, 500, 1000, 1500, 2000)
...
# True Item Parameters
true_item_param_2_5 = as.data.frame(gpcm_ip)
...
#save.image(file = "Data_GPCM.Rdata")
...
ggplot(data = plotting_theta_SE, aes(x = factor(N), y = Avg_SE, fill = collapse)) + 
  ...
  xlab("Sample Size") + 
  ylab("Average Standard Error of Recovered Person Parameters") + 
  ggtitle("Standard Errors of Recovered GPCM Person Parameters") +
  scale_fill_manual(name = "Collapse Direction", 
                    labels = c("Baseline",
                               "Collapse Down",
                               "Collapse Up"),
...
ggplot(data = plotting_theta_bias, aes(x = factor(N), y = Avg_Bias, fill = collapse)) + 
  ...
  xlab("Sample Size") + 
  ylab("Average Recovered Person Parameter Bias") + 
  ggtitle("Bias of Recovered GPCM Person Parameters") +
  scale_fill_manual(name = "Collapse Direction", 
                    labels = c("Baseline",
                               "Collapse Down",
                               "Collapse Up"),
...
ggplot(data = dplyr::filter(plotting_item_bias, Sparse == "5% Collapse Condition"), 
  ...
  xlab("Sample Size") + 
  ylab("Average Parameter Bias") + 
  ggtitle("Relative Bias of Recovered GPCM Item Parameters",
...
plotting_item_fit = dplyr::filter(plotting_item_fit, item >=7)
plotting_item_fit$item <- sub("^", "Item ", plotting_item_fit$item )
plotting_item_fit$item = as.factor(plotting_item_fit$item)
plotting_item_fit$item = factor(plotting_item_fit$item,
                                levels = c("Item 7","Item 8",
                                           "Item 9","Item 10","Item 11",
                                           "Item 12"))

ggplot(data = plotting_item_fit, 
...
ggplot(data = filter(plotting_item_fit, N != 200), 
...
ggplot(data = plotting_item_fit, 
  ...
  xlab("Sample Size") + 
  ylab("Percentage of Replications with Data Model Misift") + 
  ggtitle("GPCM Cateogry Collapse Data Model Misfit") +
  scale_fill_manual(name = "Collapse Direction", 
                    labels = c("Baseline",
                               "Collapse Down",
                               "Collapse Up"),
